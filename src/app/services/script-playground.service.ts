...
export type UntilSteadyMode =
  | 'still-life'
  | 'oscillator'
  | 'spaceship'
  | 'periodic-with-emission'
  | 'inconclusive';

export interface UntilSteadyDetection {
  mode: UntilSteadyMode;
  step: number;
  period: number;
  dx: number;
  dy: number;
...
export interface UntilSteadyDetectorConfig {
  confirmationsNeeded?: number;
  maxShipPeriod?: number;
  maxHistoryPerShape?: number;
  emissionMinHistory?: number;
  emissionMaxPeriod?: number;
  minGrowthPerPeriod?: number;
  minAreaGrowthPerPeriod?: number;
}

interface UntilSteadySnapshotSummary {
  step: number;
  population: number;
  area: number;
  anchorX: number;
  anchorY: number;
  absoluteSignature: string;
  normalizedSignature: string;
}

interface UntilSteadyCycleCandidate {
  key: string;
  mode: 'still-life' | 'oscillator' | 'spaceship';
  period: number;
  expectedGap: number;
  dx: number;
  dy: number;
...
interface UntilSteadyCandidateState {
  score: number;
  lastStep: number;
  expectedGap: number;
}

const UNTIL_STEADY_DEFAULTS: Required<UntilSteadyDetectorConfig> = {
...
export class UntilSteadyHeuristicDetector {
  private readonly cfg: Required<UntilSteadyDetectorConfig>;
  private readonly snapshotsByNormalized = new Map<string, UntilSteadySnapshotSummary[]>();
  private readonly candidates = new Map<string, UntilSteadyCandidateState>();
  private readonly populationHistory: number[] = [];
  private readonly areaHistory: number[] = [];
  private readonly stepHistory: number[] = [];

  constructor(config: UntilSteadyDetectorConfig = {}) {
    this.cfg = {
  ...
  observe(step: number, cells: Set<string>): UntilSteadyDetection | null {
  ...
  private detectCycle(summary: UntilSteadySnapshotSummary): UntilSteadyDetection | null {
    const matches = this.snapshotsByNormalized.get(summary.normalizedSignature);
    if (!Array.isArray(matches) || matches.length === 0) return null;

    let best: UntilSteadyCycleCandidate | null = null;
    ...
    for (let idx = matches.length - 1; idx >= 0; idx -= 1) {
      ...
      if (summary.absoluteSignature === previous.absoluteSignature) {
        const exactCandidate: UntilSteadyCycleCandidate = {
        ...
        const shipCandidate: UntilSteadyCycleCandidate = {
  ...
  private detectPeriodicEmission(step: number): UntilSteadyDetection | null {
  ...
  private hasRepeatingPopulationDeltas(period: number) {
  ...
  private bumpCandidate(key: string, step: number, expectedGap: number) {
  ...
  private pickPreferredCandidate(
    current: UntilSteadyCycleCandidate | null,
    next: UntilSteadyCycleCandidate
  ) {
  ...
  private storeSnapshot(summary: UntilSteadySnapshotSummary) {
  ...
  private summarizeSnapshot(step: number, cells: Set<string>): UntilSteadySnapshotSummary {
...
export class ScriptPlaygroundService {
  ...
  async runScript(scriptCode: string, context: ScriptExecutionContext): Promise<ScriptRunResult> {
    ...
    const runUntilSteady = async (varName: string, maxSteps: number, line: number) => {
      ...
      // Step 0 is the pre-command world snapshot.
      detector.observe(0, state.cells);

      let stepCount = 0;
      let detection: UntilSteadyDetection | null = null;

      while (stepCount < maxSteps && !detection) {
        throwIfAborted();
        model.step(1);
        stepCount += 1;
        consume(1);
      ...
      const mode: UntilSteadyMode = detection?.mode || 'inconclusive';
      const isSteady =
        mode === 'still-life'
        || mode === 'oscillator'
        || mode === 'spaceship';
      ...
      const detailParts = [
        `mode=${mode}`,
        `period=${state.vars[`${varName}_period`]}`,
        `dx=${state.vars[`${varName}_dx`]}`,
        `dy=${state.vars[`${varName}_dy`]}`,
        `confidence=${state.vars[`${varName}_confidence`]}`
      ];
      emitLog(`UNTIL_STEADY ${varName}=${state.vars[varName]} ${detailParts.join(' ')}`);
    };
